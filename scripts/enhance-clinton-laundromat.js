/**
 * Enhanced Laundromat Data with Google Places API - Clinton Township Fix
 * 
 * This script specifically enhances the #1 Cleaners and Super Laundromat in Clinton Township
 * with accurate Google Places API data for nearby locations.
 */
...
function log(message) {
  ...
  fs.appendFileSync('clinton-laundromat-enhancement.log', logMessage + '\n');
}
...
// Main function to enhance the Clinton Township laundromat
async function enhanceClintonLaundromat() {
  const client = await pool.connect();
  
  try {
    // Find the Clinton Township laundromat by slug
    const query = `
      SELECT id, name, latitude, longitude, city, state
      FROM laundromats
      WHERE slug = '1-cleaners-and-super-laundromat-clinton-township-michigan'
    `;
    
    const result = await client.query(query);
    
    if (result.rows.length === 0) {
      log('Clinton Township laundromat not found');
      return false;
    }
    ...
    log(`Enhancing Clinton Township laundromat ID ${id} at ${latitude},${longitude} in ${city}, ${state}`);
    ...
    // In rural areas or strip malls, try convenience stores and grocery stores too
    let bakeries = [];
    let groceries = [];
    ...
    // Get nearby activities - expanded for suburban areas
    ...
    // If we don't have enough activities, look for more options
    ...
    // In suburban areas, also look for gas stations as they're important landmarks
    let gasStations = [];
    ...
    log(`Successfully updated Clinton Township laundromat ID ${id} with nearby places data`);
    ...
    if (foodPlaces.length > 0) {
      ...
      log(`Updated Clinton Township laundromat with Google data for accurate open/closed status`);
    }
    
    return true;
  } catch (error) {
    log(`Error processing Clinton Township laundromat: ${error.message}`);
    return false;
  } finally {
    client.release();
  }
...
enhanceClintonLaundromat()
  .then(success => {
    if (success) {
      log('Clinton Township laundromat enhancement completed successfully');
    } else {
      log('Clinton Township laundromat enhancement failed');
    }
    process.exit(0);
  })
  .catch(error => {
