/**
 * Fix City-State Relationship Script
 * 
  ...
  try {
    ...
    for (const state of states) {
      ...
      const stateVariationsResult = await client.query(stateVariationsQuery, [
        state.name,
        state.abbr,
        `%${state.name}%` // Also catch partial matches like "New" for "New York"
      ]);
      ...
      for (const stateVariation of stateVariations) {
        // Skip if this is already the canonical abbr or name
        if (stateVariation === state.abbr || stateVariation === state.name) continue;
        ...
        for (const city of cities) {
          ...
          const existingCityResult = await client.query(existingCityQuery, [city.name, state.abbr, state.name]);
      ...
      const totalLaundryCountResult = await client.query(totalLaundryCountQuery, [state.abbr, state.name]);
